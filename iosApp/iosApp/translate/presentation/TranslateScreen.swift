    ...
    var body: some View {
        ZStack {
            List {
                ...
                TranslateTextField(fromText: Binding(get:{ viewModel.state.fromText}, set: {viewModel.onEvent(event: TranslateEvent.ChangeTranslationText(text: $0))}),
                                   toText: viewModel.state.toText, isTranslating: viewModel.state.isTranslating, fromLanguage: viewModel.state.fromLanguage, toLanguage: viewModel.state.toLanguage, onTranslateEvent:{
                ...
                if(!viewModel.state.history.isEmpty){
                    //the Header for the history items
                    Text("History")
                        .font(.title)
                        .bold()
